...
export class GenericHttpService<T> {
  ...
  //     if (httpHeaders != null) {
  //       httpHeaders.keys().forEach((element: any) => {
  //         this.requestHeaders = this.requestHeaders.set(
  //           element,
  //           httpHeaders.get(element)
  //         );
  //       });
  //     }
  //   }
  ...
  public post(
    ...
    return this.httpClient
      .post<T>(this.generateRequestUrl(action), item, {
  ...
  public get(
    ...
    // this.AddExtendedHeaders(httpHeaders);
    return this.httpClient
      .get<T>(this.generateRequestUrl(action + '/' + id), {
        headers: this.requestHeaders,
      })
      .pipe(map((data) => data as T));
  }
  ...
  public getList(
    ...
    if (queryOptions) {
      fullPath = `${fullPath}?${Object.keys(queryOptions)
